...
## Overview
digital-twin-team2 is a prototype Digital Twin chatbot platform that models and simulates an agent/persona using agent-based workflows, persistent state, and retrieval-augmented generation (RAG). The repository is a Next.js (App Router) TypeScript application designed to be modular, observable, and extensible. Always consult `docs/prd.md` for feature-level requirements and acceptance criteria before implementing changes.
...
## How the System Works (chat pipeline)
1. Client POSTs to `/api/chat` with a `ChatRequestPayload` containing a messages array (and optional `sessionId`).
2. The route enforces per-user rate limits via Redis (`checkRateLimit`).
3. The latest user message is embedded (`generateEmbeddings`), and top-k similar vectors are fetched from Upstash Vector (`querySimilarVectors`). Retrieved metadata is formatted as inline context.
4. Messages are normalized into Groq-compatible roles, optionally prepending a system prompt that includes the RAG context.
5. Groq inference is invoked server-side (`callGroqChat`), and the assistant reply is returned as a typed `ChatResponsePayload` with metadata flags (`ragEnabled`, `contextCount`).
6. A health check (`GET /api/chat`) returns a simple status payload.

Keep Groq and vector DB calls server-side; never expose secrets to the client.
...
2. **Navigate to the Next.js project directory:**
   - `cd digital-twin`
3. Install Node.js 18+ (recommended for Next.js 16) and npm (or pnpm/yarn if preferred).
4. Install dependencies:
   - `npm install`
5. Create a local environment file:
   - `copy .env.example .env.local`
6. Populate required env vars in `.env.local` (see next section).
...
If using pnpm or yarn, replace the npm commands accordingly.
...
- Next.js App Router:
  - Default to Server Components unless client state/hooks are required.
...
- `lib/vector.ts` — Upstash vector helper
- `lib/redis.ts` — rate limiting helper
